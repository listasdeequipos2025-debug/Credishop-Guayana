...
import {
  ...
  MessageSquare, Share2, Trash2, Edit, Download, Heart, Sparkles
} from "lucide-react";
...
export default function SalesManager({
  ...
  const handleShareWhatsApp = (sale: Sale) => {
    ...
    const mensajeCobro = isPending
      ? `\n\n📢 *MENSAJE DE COBRANZA - MONTO PENDIENTE*\n` +
        `Estimado(a) *${sale.customerName}*, le recordamos que mantiene un *saldo pendiente por cancelar de $${(sale.remainingAmount || 0).toLocaleString()}* ` +
        `relacionado con este pedido (*${sale.productName}*).\n` +
        `Agradecemos realizar su pago a la brevedad o reportar su comprobante por esta vía. ¡Muchas gracias!`
      : ``;

    const text = `*SINGLUTENPZO* 🌾🍰✨\n` +
      `*Oriven Distribuidora de Alimentos J-40633400-6*\n` +
      `*Productos 100% Sin Gluten & Postres Saludables*\n` +
      `----------------------------------------\n` +
            ...
            <div className="flex flex-col sm:flex-row gap-2 justify-end pt-2 border-t border-slate-100">
              ...
              {selectedReceiptSale.customerEmail && (
                <button
                  ...
                  className="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 disabled:bg-blue-100/50 disabled:text-blue-400 border border-blue-200 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center space-x-1.5 w-full sm:w-auto animate-fade-in"
                >
              ...
              {(!selectedReceiptSale.remainingAmount || selectedReceiptSale.remainingAmount === 0 || selectedReceiptSale.status === "pagado") && (
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white rounded-xl text-xs font-extrabold uppercase tracking-wider flex items-center justify-center space-x-1.5 shadow-md hover:shadow-lg transition-all w-full sm:w-auto animate-fade-in"
                >
                  <Heart className="h-4 w-4 text-rose-200 fill-rose-200" />
                  <span>Agradecer WhatsApp</span>
                </button>
              )}
              <button
                onClick={() => window.print()}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-extrabold uppercase tracking-wider rounded-xl shadow-md flex items-center justify-center space-x-1.5 w-full sm:w-auto"
              >
