...
export default function Dashboard({
  ...
  const generatePDFReport = () => {
    const doc = new jsPDF();
    const primaryColor: [number, number, number] = [109, 40, 217]; // Purple
    ...
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center space-x-1 bg-white px-2.5 py-1.5 rounded-2xl border border-slate-200 shadow-sm">
            <button
              ...
              className="p-1 text-slate-400 hover:text-purple-600 hover:bg-slate-50 rounded-lg transition-colors text-xs font-black"
              title="Año Anterior"
            >
              ◀
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          <div className="absolute right-0 top-0 bg-purple-50 p-6 rounded-bl-3xl group-hover:bg-purple-100/50 transition-colors">
        ...
        {/* Sales Gross Profit */}
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          <div className="absolute right-0 top-0 bg-indigo-50 p-6 rounded-bl-3xl group-hover:bg-indigo-100/50 transition-colors">
            <TrendingUp className="h-6 w-6 text-indigo-600" />
          </div>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Utilidad Bruta Ventas</p>
          <h3 className="text-2xl font-black text-indigo-950 mt-2">${salesGrossProfit.toLocaleString()}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-900 text-white p-6 rounded-3xl border border-slate-800 shadow-xl flex flex-col justify-between">
          <div>
            <span className="text-[10px] bg-purple-500/20 text-purple-300 font-extrabold uppercase px-2.5 py-1 rounded-md border border-purple-400/20">
              Cuentas por Pagar (Crédito Proveedores)
            </span>
            <h4 className="text-3xl font-black text-purple-200 mt-4">${outstandingDebts.toLocaleString()}</h4>
      ...
      <div className="flex justify-end pt-4">
        <button
          ...
          className="flex items-center space-x-2 px-6 py-3.5 bg-purple-700 hover:bg-purple-600 active:bg-purple-800 text-white font-extrabold text-sm uppercase tracking-wider rounded-2xl shadow-lg hover:shadow-purple-700/20 active:scale-[0.98] transition-all"
        >
